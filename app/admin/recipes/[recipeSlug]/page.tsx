...
const ModifyRecipe = ({params}: {params: {recipeSlug: string}}) => {
    ...
    const modifyRecipe = async (e) => {
        ...
        try{
            ...
            let recipePicture = await uploadRecipeImage();
            if(recipePicture === undefined){
                recipePicture = picture
            }
  
            console.log("la vraie url de l'image est : "+recipePicture)
            console.log("mon titre est : "+title)
            ...
            if (response.ok) {
                toast.success("Recipe udpated successfully !");
                try {
                    await router.push(`/admin/recipes/${newSlug}`);
                } catch (err) {
  ...
  return (
    ...
    <>
      <h1 className="text-3xl text-white ml-3 text-center">Recipe : {title}</h1>
      <div><Toaster/></div>
